...
if.then4.i:                                       ; preds = %if.end.i
  ...
  br i1 %cmp8.i, label %cleanup12, label %if.then.loopexit
...
MtThread_Process.exit.thread47:                   ; preds = %if.end59.i, %if.end50.i, %if.end47.i, %cleanup.cont40.i, %if.end26.i, %cleanup.cont.i, %FullRead.exit.i
  ...
  %sub.le73 = add i32 %2, -1
  %cmp.le = icmp eq i32 %1, %sub.le73
  %add.le = add i32 %1, 1
  %spec.select.le62 = select i1 %cmp.le, i32 0, i32 %add.le
  call void @llvm.lifetime.end.p0(i64 8, ptr nonnull %destSize.i) #6
  br label %if.then

if.then.loopexit:                                 ; preds = %for.cond, %if.then4.i
  %sub.le = add i32 %2, -1
  %cmp.le69 = icmp eq i32 %1, %sub.le
  %add.le66 = add i32 %1, 1
  %spec.select.le = select i1 %cmp.le69, i32 0, i32 %add.le66
  br label %if.then

if.then:                                          ; preds = %if.then.loopexit, %MtThread_Process.exit.thread47
  %idxprom57.in = phi i32 [ %spec.select.le62, %MtThread_Process.exit.thread47 ], [ %spec.select.le, %if.then.loopexit ]
  %retval.3.i39 = phi i32 [ %retval.2.i.ph, %MtThread_Process.exit.thread47 ], [ 12, %if.then.loopexit ]
  %idxprom57 = zext i32 %idxprom57.in to i64
...
cleanup.thread:                                   ; preds = %if.then.i32, %MtCoder_SetError.exit
  ...
  %stopReading = getelementptr inbounds %struct._CMtCoder, ptr %0, i64 0, i32 11, i64 %idxprom57, i32 7
  store i32 1, ptr %stopReading, align 8, !tbaa !54
  %stopWriting = getelementptr inbounds %struct._CMtCoder, ptr %0, i64 0, i32 11, i64 %idxprom57, i32 8
  store i32 1, ptr %stopWriting, align 4, !tbaa !55
  %canRead = getelementptr inbounds %struct._CMtCoder, ptr %0, i64 0, i32 11, i64 %idxprom57, i32 9
  %call6 = call i32 @Event_Set(ptr noundef nonnull %canRead) #6
  %canWrite = getelementptr inbounds %struct._CMtCoder, ptr %0, i64 0, i32 11, i64 %idxprom57, i32 10
