...
for.cond.preheader:                               ; preds = %if.end
  %call11124 = call i32 @cli_readn(i32 noundef %fd, ptr noundef nonnull %buffer, i32 noundef 4) #6
  %cmp12.not125 = icmp eq i32 %call11124, 4
  br i1 %cmp12.not125, label %if.end15.lr.ph, label %cleanup
...
if.end26:                                         ; preds = %if.end15
  ...
  br i1 %brmerge, label %cleanup.loopexit.split.loop.exit, label %if.end60
...
cleanup.loopexit.split.loop.exit:                 ; preds = %if.end26
  %.mux.le = select i1 %or.cond121, i32 1, i32 -1
  br label %cleanup

cleanup:                                          ; preds = %cleanup.loopexit.split.loop.exit, %if.end88, %jpeg_check_photoshop.exit, %if.end66, %if.end60, %for.cond.backedge, %for.cond.preheader, %if.end, %entry
  %retval.0 = phi i32 [ 0, %entry ], [ 0, %if.end ], [ 0, %for.cond.preheader ], [ %.mux.le, %cleanup.loopexit.split.loop.exit ], [ -1, %if.end88 ], [ %retval.0.i118, %jpeg_check_photoshop.exit ], [ 1, %if.end66 ], [ 0, %if.end60 ], [ 0, %for.cond.backedge ]
