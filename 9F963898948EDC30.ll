...
do.body.preheader:                                ; preds = %if.end16
  ...
  %call.i425 = call i64 @fread(ptr noundef nonnull %part, i64 noundef 1, i64 noundef 1, ptr noundef nonnull %call3)
  %cmp.not.i426 = icmp eq i64 %call.i425, 1
  br i1 %cmp.not.i426, label %if.end.i, label %sw.bb

if.then19:                                        ; preds = %if.end16
  %call20 = tail call i32 @fclose(ptr noundef nonnull %call3)
  br label %cleanup85

if.end.i:                                         ; preds = %do.body.preheader, %cleanup
  %fb.0427 = phi ptr [ %fb.6, %cleanup ], [ null, %do.body.preheader ]
...
sw.bb:                                            ; preds = %cleanup, %if.end.i, %do.body.preheader, %if.then13.i
  %fb.0390 = phi ptr [ %fb.0427, %if.then13.i ], [ null, %do.body.preheader ], [ %fb.0427, %if.end.i ], [ %fb.6, %cleanup ]
...
sw.bb35:                                          ; preds = %if.end34
  ...
  %cmp36.not = icmp eq ptr %fb.0427, null
  br i1 %cmp36.not, label %if.end39, label %if.then38

if.then38:                                        ; preds = %sw.bb35
  call void @fileblobDestroy(ptr noundef nonnull %fb.0427) #8
  br label %if.end39
...
if.end16.i:                                       ; preds = %if.end9.i
  ...
  %cmp17.i = icmp eq ptr %fb.0427, null
...
if.end25.i:                                       ; preds = %if.then19.i, %if.end16.i
  %5 = phi ptr [ %call20.i, %if.then19.i ], [ %fb.0427, %if.end16.i ]
  call void @fileblobSetFilename(ptr noundef nonnull %5, ptr noundef %dir, ptr noundef nonnull %call5.i130) #8
  call void @free(ptr noundef nonnull %call5.i130) #8
  br label %sw.epilog.i135

sw.bb26.i:                                        ; preds = %sw.bb46
  %cmp27.i132 = icmp eq ptr %fb.0427, null
...
if.end35.i:                                       ; preds = %if.then29.i, %sw.bb26.i
  %fb.2 = phi ptr [ %call30.i, %if.then29.i ], [ %fb.0427, %sw.bb26.i ]
  br label %for.body.i
...
sw.epilog.i135:                                   ; preds = %for.inc.i, %sw.default.i, %cleanup.i, %if.end25.i
  %fb.3 = phi ptr [ %fb.0427, %sw.default.i ], [ %fb.2, %cleanup.i ], [ %5, %if.end25.i ], [ %fb.2, %for.inc.i ]
...
tnef_attachment.exit.thread:                      ; preds = %if.end68.i, %if.then29.i, %if.end.i131, %if.then67.i, %if.then23.i, %if.then15.i
  %fb.4.ph = phi ptr [ %fb.0427, %if.then15.i ], [ null, %if.then23.i ], [ %fb.3, %if.then67.i ], [ %fb.3, %if.end68.i ], [ null, %if.then29.i ], [ %fb.0427, %if.end.i131 ]
...
sw.default52:                                     ; preds = %if.end34
  ...
  %6 = load i8, ptr @cli_debug_flag, align 1, !tbaa !13
  %tobool55.not = icmp eq i8 %6, 0
...
if.then61:                                        ; preds = %if.then56
  ...
  %call63257 = call i32 @cli_readn(i32 noundef %desc, ptr noundef nonnull %buffer, i32 noundef 8192) #8
  %cmp64258 = icmp sgt i32 %call63257, 0
  br i1 %cmp64258, label %while.body, label %while.end

while.body:                                       ; preds = %if.then61, %while.body
  %call63259 = phi i32 [ %call63, %while.body ], [ %call63257, %if.then61 ]
  %call67 = call i32 @cli_writen(i32 noundef %call58, ptr noundef nonnull %buffer, i32 noundef %call63259) #8
...
cleanup:                                          ; preds = %tnef_message.exit, %tnef_attachment.exit, %sw.epilog
  %fb.6 = phi ptr [ %fb.0427, %sw.epilog ], [ %fb.3, %tnef_attachment.exit ], [ %call40, %tnef_message.exit ]
...
do.end:                                           ; preds = %sw.bb, %if.then24, %sw.default, %sw.default52, %if.end69, %if.then44, %if.then50, %if.then30
  %fb.6.ph = phi ptr [ %fb.0427, %sw.default52 ], [ %fb.0427, %if.end69 ], [ %call40, %if.then44 ], [ %fb.4182, %if.then50 ], [ %fb.0427, %if.then30 ], [ %fb.0427, %sw.default ], [ %fb.0390, %if.then24 ], [ %fb.0390, %sw.bb ]
