...
if.end.i:                                         ; preds = %if.else.i.i
  ...
  %14 = tail call float @llvm.fmuladd.f32(float %12, float %conv42.i.i, float %mul65.i.i)
  %fneg66.i.i = fneg float %14
  %conv36.i.i = fdiv float %9, %11
  %mul58.i.i = fmul float %conv48.i.i, %13
  %15 = tail call float @llvm.fmuladd.f32(float %12, float %conv36.i.i, float %mul58.i.i)
  %fneg59.i.i = fneg float %15
...
if.then.i125:                                     ; preds = %entry
  ...
  %25 = load float, ptr %tx.i, align 8, !tbaa !12
  %26 = load float, ptr %ty.i, align 8, !tbaa !14
...
if.end:                                           ; preds = %cleanup.thread.i140, %if.then.i125
  %74 = phi <2 x float> [ %35, %if.then.i125 ], [ %73, %cleanup.thread.i140 ]
  %75 = phi <2 x double> [ %39, %if.then.i125 ], [ %64, %cleanup.thread.i140 ]
  %76 = phi <2 x double> [ %43, %if.then.i125 ], [ %71, %cleanup.thread.i140 ]
  %77 = shufflevector <2 x float> %74, <2 x float> poison, <4 x i32> <i32 0, i32 1, i32 0, i32 1>
  %78 = fptrunc <2 x double> %75 to <2 x float>
  %79 = fptrunc <2 x double> %76 to <2 x float>
  ...
  %83 = shufflevector <4 x float> %82, <4 x float> <float poison, float poison, float -0.000000e+00, float -0.000000e+00>, <4 x i32> <i32 0, i32 1, i32 6, i32 7>
  %84 = shufflevector <4 x float> <float -0.000000e+00, float -0.000000e+00, float poison, float poison>, <4 x float> %82, <4 x i32> <i32 0, i32 1, i32 6, i32 7>
  %85 = select <4 x i1> %81, <4 x float> %83, <4 x float> %84
  %86 = fadd <4 x float> %77, %85
  %87 = fcmp olt <2 x float> %79, zeroinitializer
  %88 = shufflevector <2 x i1> %87, <2 x i1> poison, <4 x i32> <i32 0, i32 1, i32 0, i32 1>
  %89 = shufflevector <2 x float> %79, <2 x float> poison, <4 x i32> <i32 0, i32 1, i32 0, i32 1>
  %90 = shufflevector <4 x float> %89, <4 x float> <float poison, float poison, float -0.000000e+00, float -0.000000e+00>, <4 x i32> <i32 0, i32 1, i32 6, i32 7>
  %91 = shufflevector <4 x float> <float -0.000000e+00, float -0.000000e+00, float poison, float poison>, <4 x float> %89, <4 x i32> <i32 0, i32 1, i32 6, i32 7>
  %92 = select <4 x i1> %88, <4 x float> %90, <4 x float> %91
  %93 = fadd <4 x float> %86, %92
  store <4 x float> %93, ptr %pbox_out, align 4, !tbaa !6
  br label %cleanup
