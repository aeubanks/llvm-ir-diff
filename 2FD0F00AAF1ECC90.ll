...
if.then:                                          ; preds = %entry
  ...
  %call = tail call noalias ptr @calloc(i64 noundef %conv, i64 noundef %conv1) #9
...
define dso_local noalias ptr @hypre_ReAlloc(ptr nocapture noundef %ptr, i32 noundef %size) local_unnamed_addr #4 {
entry:
  %conv = sext i32 %size to i64
  %call = tail call ptr @realloc(ptr noundef %ptr, i64 noundef %conv) #10
...
if.then:                                          ; preds = %entry
  tail call void @free(ptr noundef nonnull %ptr) #11
  br label %if.end
