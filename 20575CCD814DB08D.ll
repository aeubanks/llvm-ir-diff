...
for.body47.lr.ph:                                 ; preds = %if.end, %if.end12
  ...
  %add38 = sub nsw i32 1, %n
  %mul39 = mul nsw i32 %add38, %incy
...
for.body37.lr.ph:                                 ; preds = %if.end, %if.end7
  ...
  %add29 = sub nsw i32 1, %n
  %mul30 = mul nsw i32 %add29, %incy
...
land.rhs:                                         ; preds = %land.rhs.lr.ph, %while.body
  %sx.addr.0284 = phi ptr [ %sx, %land.rhs.lr.ph ], [ %add.ptr, %while.body ]
  %i.0283 = phi i32 [ 0, %land.rhs.lr.ph ], [ %inc, %while.body ]
  %0 = load float, ptr %sx.addr.0284, align 4, !tbaa !5
  %cmp8 = fcmp oeq float %0, 0.000000e+00
  br i1 %cmp8, label %while.body, label %while.end

while.body:                                       ; preds = %land.rhs
  %inc = add nuw nsw i32 %i.0283, 1
  %add.ptr = getelementptr inbounds float, ptr %sx.addr.0284, i64 %idx.ext
  %exitcond.not = icmp eq i32 %inc, %n
  br i1 %exitcond.not, label %cleanup, label %land.rhs, !llvm.loop !52

while.end:                                        ; preds = %land.rhs
  %cmp10.not = icmp slt i32 %i.0283, %n
  br i1 %cmp10.not, label %START.preheader, label %cleanup

START.preheader:                                  ; preds = %while.end
  %1 = load float, ptr %sx.addr.0284, align 4, !tbaa !5
  %cmp15299 = fcmp ogt float %1, 0.000000e+00
  %fneg300 = fneg float %1
  %cond301 = select i1 %cmp15299, float %1, float %fneg300
  %conv17302 = fpext float %cond301 to double
  %cmp18303 = fcmp ogt double %conv17302, 0x3BFB084B92366CC2
  br i1 %cmp18303, label %for.cond.preheader, label %if.else

for.cond.preheader.loopexit:                      ; preds = %for.body56
  %conv69 = fpext float %xmax.0292 to double
  %mul70 = fmul double %sum.2291, %conv69
...
for.cond.preheader:                               ; preds = %for.cond.preheader.loopexit, %START.preheader
  ...
  %i.1.lcssa = phi i32 [ %i.0283, %START.preheader ], [ %i.3294, %for.cond.preheader.loopexit ]
  %sx.addr.1.lcssa = phi ptr [ %sx.addr.0284, %START.preheader ], [ %sx.addr.3293, %for.cond.preheader.loopexit ]
  %cmp21312 = icmp slt i32 %i.1.lcssa, %n
  br i1 %cmp21312, label %for.body.lr.ph, label %for.end

for.body.lr.ph:                                   ; preds = %for.cond.preheader
  %idx.ext38 = zext i32 %incx to i64
  br label %for.body

for.body:                                         ; preds = %for.body.lr.ph, %if.end35
  %sx.addr.2315 = phi ptr [ %sx.addr.1.lcssa, %for.body.lr.ph ], [ %add.ptr39, %if.end35 ]
  %i.2314 = phi i32 [ %i.1.lcssa, %for.body.lr.ph ], [ %inc37, %if.end35 ]
  %sum.1313 = phi double [ %sum.0.lcssa, %for.body.lr.ph ], [ %add, %if.end35 ]
  %2 = load float, ptr %sx.addr.2315, align 4, !tbaa !5
  %cmp24 = fcmp ogt float %2, 0.000000e+00
  %fneg28 = fneg float %2
  %cond30 = select i1 %cmp24, float %2, float %fneg28
...
if.end35:                                         ; preds = %for.body
  ...
  %add = fadd double %sum.1313, %conv36
  %inc37 = add i32 %i.2314, 1
  %add.ptr39 = getelementptr inbounds float, ptr %sx.addr.2315, i64 %idx.ext38
  %exitcond341.not = icmp eq i32 %inc37, %n
  br i1 %exitcond341.not, label %for.end, label %for.body, !llvm.loop !53
...
if.else:                                          ; preds = %START.preheader
  %idx.ext49 = zext i32 %incx to i64
  %i.3289 = add nuw nsw i32 %i.0283, 1
  %cmp54290 = icmp slt i32 %i.3289, %n
  br i1 %cmp54290, label %for.body56, label %for.end109

for.body56:                                       ; preds = %if.else, %for.inc105
  %i.3294 = phi i32 [ %i.3, %for.inc105 ], [ %i.3289, %if.else ]
  %sx.addr.3293.pn = phi ptr [ %sx.addr.3293, %for.inc105 ], [ %sx.addr.0284, %if.else ]
  %xmax.0292 = phi float [ %xmax.1, %for.inc105 ], [ %cond301, %if.else ]
  %sum.2291 = phi double [ %sum.3, %for.inc105 ], [ 1.000000e+00, %if.else ]
  %sx.addr.3293 = getelementptr inbounds float, ptr %sx.addr.3293.pn, i64 %idx.ext49
  %3 = load float, ptr %sx.addr.3293, align 4, !tbaa !5
  %cmp58 = fcmp ogt float %3, 0.000000e+00
  %fneg62 = fneg float %3
  %cond64 = select i1 %cmp58, float %3, float %fneg62
...
if.end73:                                         ; preds = %for.body56
  %cmp82 = fcmp ogt float %cond64, %xmax.0292
  br i1 %cmp82, label %if.then84, label %if.end99

if.then84:                                        ; preds = %if.end73
  %div85 = fdiv float %xmax.0292, %3
  %conv86 = fpext float %div85 to double
  %mul87 = fmul double %sum.2291, %conv86
  %4 = tail call double @llvm.fmuladd.f64(double %mul87, double %conv86, double 1.000000e+00)
  br label %for.inc105

if.end99:                                         ; preds = %if.end73
  %div100 = fdiv float %3, %xmax.0292
  %mul102 = fmul float %div100, %div100
  %conv103 = fpext float %mul102 to double
  %add104 = fadd double %sum.2291, %conv103
  br label %for.inc105

for.inc105:                                       ; preds = %if.end99, %if.then84
  %sum.3 = phi double [ %4, %if.then84 ], [ %add104, %if.end99 ]
  %xmax.1 = phi float [ %cond64, %if.then84 ], [ %xmax.0292, %if.end99 ]
  %i.3 = add i32 %i.3294, 1
  %exitcond340.not = icmp eq i32 %i.3, %n
  br i1 %exitcond340.not, label %for.end109.loopexit, label %for.body56, !llvm.loop !54

for.end109.loopexit:                              ; preds = %for.inc105
  %.pre342 = fpext float %xmax.1 to double
  br label %for.end109

for.end109:                                       ; preds = %if.else, %for.end109.loopexit
  %conv110.pre-phi = phi double [ %.pre342, %for.end109.loopexit ], [ %conv17302, %if.else ]
...
GOT_LARGE:                                        ; preds = %for.body
  %conv23.le = fpext float %2 to double
  %div114 = fdiv double %sum.1313, %conv23.le
  %div116 = fdiv double %div114, %conv23.le
  %add117 = fadd double %div116, 1.000000e+00
  %i.4318 = add nsw i32 %i.2314, 1
  %cmp130319 = icmp slt i32 %i.4318, %n
  br i1 %cmp130319, label %for.body132, label %for.end168

for.body132:                                      ; preds = %GOT_LARGE, %for.inc164
  %i.4323 = phi i32 [ %i.4, %for.inc164 ], [ %i.4318, %GOT_LARGE ]
  %sx.addr.4322.pn = phi ptr [ %sx.addr.4322, %for.inc164 ], [ %sx.addr.2315, %GOT_LARGE ]
  %xmax.2321 = phi float [ %xmax.3, %for.inc164 ], [ %cond30, %GOT_LARGE ]
  %sum.4320 = phi double [ %sum.5, %for.inc164 ], [ %add117, %GOT_LARGE ]
  %sx.addr.4322 = getelementptr inbounds float, ptr %sx.addr.4322.pn, i64 %idx.ext38
  %5 = load float, ptr %sx.addr.4322, align 4, !tbaa !5
  %cmp134 = fcmp ogt float %5, 0.000000e+00
  %fneg138 = fneg float %5
  %cond140 = select i1 %cmp134, float %5, float %fneg138
  %cmp141 = fcmp ogt float %cond140, %xmax.2321
  br i1 %cmp141, label %if.then143, label %if.end158

if.then143:                                       ; preds = %for.body132
  %div144 = fdiv float %xmax.2321, %5
  %conv145 = fpext float %div144 to double
  %mul146 = fmul double %sum.4320, %conv145
  %6 = tail call double @llvm.fmuladd.f64(double %mul146, double %conv145, double 1.000000e+00)
  br label %for.inc164

if.end158:                                        ; preds = %for.body132
  %div159 = fdiv float %5, %xmax.2321
  %mul161 = fmul float %div159, %div159
  %conv162 = fpext float %mul161 to double
  %add163 = fadd double %sum.4320, %conv162
  br label %for.inc164

for.inc164:                                       ; preds = %if.end158, %if.then143
  %sum.5 = phi double [ %6, %if.then143 ], [ %add163, %if.end158 ]
  %xmax.3 = phi float [ %cond140, %if.then143 ], [ %xmax.2321, %if.end158 ]
  %i.4 = add nsw i32 %i.4323, 1
...
cleanup:                                          ; preds = %while.body, %while.end, %entry, %for.end168, %for.end109, %for.end
  %retval.0 = phi double [ %mul171, %for.end168 ], [ %call40, %for.end ], [ %mul112, %for.end109 ], [ 0.000000e+00, %entry ], [ 0.000000e+00, %while.end ], [ 0.000000e+00, %while.body ]
  ret double %retval.0
}
