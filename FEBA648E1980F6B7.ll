...
define dso_local nonnull ptr @LOOK_UP_OP(ptr nocapture noundef readonly %OP) local_unnamed_addr #0 {
entry:
  br label %while.body

while.body:                                       ; preds = %entry, %if.end
  %HIGH.016 = phi i32 [ 58, %entry ], [ %HIGH.1, %if.end ]
  %LOW.015 = phi i32 [ 0, %entry ], [ %LOW.1, %if.end ]
  %add = add nsw i32 %HIGH.016, %LOW.015
  %div = sdiv i32 %add, 2
  %idxprom = sext i32 %div to i64
  %arrayidx = getelementptr inbounds [60 x %struct.OP_ENTRY], ptr @OP_TABLE, i64 0, i64 %idxprom
  %0 = load ptr, ptr %arrayidx, align 16, !tbaa !5
  %call = tail call i32 @strcmp(ptr noundef nonnull dereferenceable(1) %0, ptr noundef nonnull dereferenceable(1) %OP) #2
...
if.end:                                           ; preds = %while.body
  ...
  %LOW.1 = select i1 %cmp3, i32 %LOW.015, i32 %add5
  %HIGH.1 = select i1 %cmp3, i32 %sub, i32 %HIGH.016
  %cmp.not = icmp sgt i32 %LOW.1, %HIGH.1
  br i1 %cmp.not, label %cleanup, label %while.body, !llvm.loop !11

cleanup:                                          ; preds = %if.end, %while.body
  %retval.0 = phi ptr [ %arrayidx, %while.body ], [ getelementptr inbounds ([60 x %struct.OP_ENTRY], ptr @OP_TABLE, i64 0, i64 59), %if.end ]
  ret ptr %retval.0
}
