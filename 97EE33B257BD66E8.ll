...
for.body.lr.ph:                                   ; preds = %entry, %cond.end
  ...
  %.mux613 = select i1 %tobool6.not, i32 %width, i32 1
  br label %for.body

for.body:                                         ; preds = %for.body.lr.ph, %for.inc
  %pel.0607 = phi i32 [ 0, %for.body.lr.ph ], [ %inc, %for.inc ]
  %ap.0606 = phi i32 [ 0, %for.body.lr.ph ], [ %ap.2, %for.inc ]
  %aq.0605 = phi i32 [ 0, %for.body.lr.ph ], [ %aq.2, %for.inc ]
  %cond11 = select i1 %tobool6.not, i32 %pel.0607, i32 %shl9
  %cond22 = select i1 %tobool6.not, i32 %cond19, i32 %pel.0607
...
cond.true43:                                      ; preds = %land.lhs.true40
  %shl44 = shl nuw i32 %pel.0607, 1
  br label %cond.end50

cond.false45:                                     ; preds = %land.lhs.true40, %cond.true37
  %11 = shl nuw i32 %pel.0607, 1
  %shl46 = and i32 %11, -4
  %and = and i32 %pel.0607, 1
  %add = or i32 %shl46, %and
  br label %cond.end50

cond.end50:                                       ; preds = %lor.end, %cond.true43, %cond.false45
  %cond51 = phi i32 [ %shl44, %cond.true43 ], [ %add, %cond.false45 ], [ %pel.0607, %lor.end ]
...
if.else315:                                       ; preds = %if.end190.thread, %if.end190
  ...
  %ap.1595 = phi i32 [ %conv189, %if.end190.thread ], [ %ap.0606, %if.end190 ]
  %aq.1593 = phi i32 [ %conv184, %if.end190.thread ], [ %aq.0605, %if.end190 ]
  %add318 = add nsw i32 %mul167581, 1
  %add320 = add nsw i32 %ap.1595, %mul167581
...
for.inc:                                          ; preds = %lor.lhs.false, %if.then158, %cond.end307, %if.then196, %if.end358, %if.then360, %if.else375, %if.then163, %cond.end110
  %aq.2 = phi i32 [ %aq.0605, %if.then196 ], [ %and216, %cond.end307 ], [ %aq.1593, %if.else375 ], [ 1, %if.then360 ], [ 0, %if.end358 ], [ %aq.0605, %if.then163 ], [ %aq.0605, %if.then158 ], [ %aq.0605, %cond.end110 ], [ %aq.0605, %lor.lhs.false ]
  %ap.2 = phi i32 [ %ap.0606, %if.then196 ], [ %and217, %cond.end307 ], [ %ap.1595, %if.else375 ], [ %ap.1595, %if.then360 ], [ %ap.1595, %if.end358 ], [ %ap.0606, %if.then163 ], [ %ap.0606, %if.then158 ], [ %ap.0606, %cond.end110 ], [ %ap.0606, %lor.lhs.false ]
  %inc = add nuw nsw i32 %pel.0607, 1
  %exitcond.not = icmp eq i32 %inc, %cond612
