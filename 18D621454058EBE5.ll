...
entry:
  ...
  %1 = and i8 %0, -33
  %2 = add i8 %1, -65
  %or.cond54 = icmp ult i8 %2, 9
  br i1 %or.cond54, label %land.lhs.true8, label %return
...
land.lhs.true8:                                   ; preds = %entry
  ...
  br i1 %or.cond51, label %if.then, label %return

if.then:                                          ; preds = %land.lhs.true8
  br i1 %cmp, label %if.end31, label %if.else

if.else:                                          ; preds = %if.then
  %cmp26 = icmp ult i8 %0, 74
  br i1 %cmp26, label %if.end31, label %return

if.end31:                                         ; preds = %if.else, %if.then
  %.sink = phi i32 [ -97, %if.then ], [ -65, %if.else ]
  %sub29 = add nsw i32 %.sink, %conv
  %add = add nuw nsw i32 %sub29, 1
  %add35 = add nsw i32 %conv9, -48
  %call = tail call noundef i32 @_Z16getHexxagonIndexii(i32 noundef %add, i32 noundef %add35)
  br label %return

return:                                           ; preds = %entry, %land.lhs.true8, %if.end31, %if.else
  %retval.1 = phi i32 [ %call, %if.end31 ], [ -1, %if.else ], [ -1, %land.lhs.true8 ], [ -1, %entry ]
  ret i32 %retval.1
}
...
land.lhs.true269:                                 ; preds = %if.else261, %if.else242, %if.else242
...
if.then294:                                       ; preds = %land.lhs.true286, %land.lhs.true286
  ...
  %call.i629686 = tail call noundef i32 @_ZN13HexxagonBoard9endOfGameEv(ptr noundef nonnull align 4 dereferenceable(16) %30)
  %tobool297.not687 = icmp eq i32 %call.i629686, 0
  br i1 %tobool297.not687, label %while.body298, label %cleanup
...
if.end354.thread:                                 ; preds = %if.then322
  ...
  %cmp357682 = icmp ne i8 %conv337, -1
  %cmp361684 = icmp ne i8 %conv340, -1
  %or.cond685 = and i1 %cmp357682, %cmp361684
  br i1 %or.cond685, label %if.then362, label %if.then377
