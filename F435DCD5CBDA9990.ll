...
for.body:                                         ; preds = %entry, %cleanup293
  %i.0451 = phi i64 [ 0, %entry ], [ %inc, %cleanup293 ]
  %add.i.i401.lcssa442450 = phi i64 [ %simple_rand.seed.promoted, %entry ], [ %add.i.i401, %cleanup293 ]
  br label %for.cond.i

for.cond.i:                                       ; preds = %if.else.i, %for.body
  %add.i22.i = phi i64 [ %add.i.i401.lcssa442450, %for.body ], [ %add.i.i, %if.else.i ]
...
cleanup.cont65:                                   ; preds = %save_time
  ...
  %or.cond303.not = or i1 %cmp73, %cmp76
...
cleanup.cont110:                                  ; preds = %if.end79
  %conv117453 = and i64 %retval.0.i416, 65535
  %cmp118.not = icmp eq i64 %conv117453, 0
...
if.end201:                                        ; preds = %cleanup.cont150
  %conv208454 = and i64 %retval.0.i416, 255
  %cmp209.not = icmp eq i64 %conv208454, 0
...
cleanup.cont241:                                  ; preds = %if.end201
  ...
  %rem254435 = srem i16 %rem254.lhs.trunc, %rem254.rhs.trunc
  %rem254.sext = sext i16 %rem254435 to i32
  %cond265 = tail call i32 @llvm.abs.i32(i32 %rem254.sext, i1 true)
  %cond275 = tail call i32 @llvm.abs.i32(i32 %conv249, i1 true)
  %cmp278.not = icmp ult i32 %cond265, %cond275
  br i1 %cmp278.not, label %lor.lhs.false280, label %if.then291

lor.lhs.false280:                                 ; preds = %cleanup.cont241
  %narrow = sub nsw i16 0, %rem254435
  %.sext.neg452 = zext i16 %narrow to i32
  %mul283 = add i32 %rem254.sext, %conv
  %add285 = add i32 %mul283, %.sext.neg452
  %sext424 = shl i32 %add285, 24
  %conv287 = ashr exact i32 %sext424, 24
  %cmp289.not = icmp eq i32 %conv287, %conv248
  br i1 %cmp289.not, label %cleanup293, label %if.then291

if.then291:                                       ; preds = %lor.lhs.false280, %cleanup.cont241
  store i64 %add.i.i401, ptr @simple_rand.seed, align 8, !tbaa !5
  tail call void @abort() #5
  unreachable

cleanup293:                                       ; preds = %if.end201, %cleanup.cont110, %cleanup.cont65, %save_time, %cleanup.cont, %random_bitstring.exit417, %lor.lhs.false280
