...
if.then331:                                       ; preds = %for.cond.cleanup, %if.then324
...
if.end:                                           ; preds = %entry, %entry, %entry, %entry
  ...
  %3 = load i32, ptr %mFrameSize, align 4, !tbaa !25
  %cmp30.not = icmp eq i32 %3, %numSamples
  %4 = add nsw i32 %conv, -16
  %5 = tail call i32 @llvm.fshl.i32(i32 %conv, i32 %4, i32 30)
...
if.then201:                                       ; preds = %for.inc116.1, %if.then194
  %status.1435 = phi i32 [ 0, %for.inc116.1 ], [ %call189, %if.then194 ]
  call void @BitBufferWrite(ptr noundef %bitstream, i32 noundef 0, i32 noundef 12)
...
cleanup255:                                       ; preds = %for.body247, %for.body236, %for.body225, %for.body214, %if.end181, %for.cond244.preheader, %sw.bb232, %sw.bb221, %for.cond211.preheader, %cleanup, %if.end207, %entry
  %retval.0 = phi i32 [ -50, %entry ], [ %call.lcssa, %cleanup ], [ %status.1435, %if.end207 ], [ %status.1435, %for.cond211.preheader ], [ %status.1435, %sw.bb221 ], [ %status.1435, %sw.bb232 ], [ %status.1435, %for.cond244.preheader ], [ %call189, %if.end181 ], [ %status.1435, %for.body214 ], [ %status.1435, %for.body225 ], [ %status.1435, %for.body236 ], [ %status.1435, %for.body247 ]
  call void @llvm.lifetime.end.p0(i64 4, ptr nonnull %bits1) #15
  call void @llvm.lifetime.end.p0(i64 36, ptr nonnull %agParams) #15
  call void @llvm.lifetime.end.p0(i64 24, ptr nonnull %startBits) #15
  ret i32 %retval.0
